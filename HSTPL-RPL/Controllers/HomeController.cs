...
using HSTPL.RPL.Models.UserModels;
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                HSTPL.RPL.Data.Models.Users user = new Users
                {
                    Password = new HSTPL.RPL.Helpers.PasswordEncode().Encoder(model.Password), // SHA256
                    Email = model.Email,
                    Name = model.Name,
        ...
        public async Task<ActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                HSTPL.RPL.Data.Models.Users user = _userService.GetByFilter(i => i.Email == model.Email && i.Password == new PasswordEncode().Encoder(model.Password));
                if (user == null)
                {
